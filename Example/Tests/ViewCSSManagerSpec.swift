    ...
    override func spec() {
        ...
        describe("#getConfig") {
            ...
            describe("parent") {
                it("class") {
                    manager!.setCSS(dict: css)
                    let config = manager!.getConfig(className: "ui_button", class: nil, style: nil,
                                                    parentClass: "class", parentStyle: nil)
                    expect(config.background?.color?.toCSS).to(equal("#0000FFFF"))
                }
                
                it("style") {
                    manager!.setCSS(dict: css)
                    let config = manager!.getConfig(className: "ui_button", class: nil, style: "background-color : #00FF00FF;",
                                                    parentClass: "class", parentStyle: nil)
                    expect(config.background?.color?.toCSS).to(equal("#00FF00FF"))
                }
            }
